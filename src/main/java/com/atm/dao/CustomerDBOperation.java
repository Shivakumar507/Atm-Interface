...
public class CustomerDBOperation {
	...
	public void withdraw(long cid,double withdrawAmount) 
	{
		...
		if(withdrawAmount<=availableBalance)
		{
			...
			PreparedStatement ps1=conn.prepareStatement("update Customer set available_balance=? where acc_id=?");
	...
	{
		PreparedStatement ps=conn.prepareStatement("update Customer set Pin=? where c_id=? ");
